...
const NavItem = ({ to, children, options, onClick }) => {
  return (
    <li>
      <NavLink
...
export default function Navbar() {
  ...
  const handleScan = (scannedData) => {
    const [type, id] = scannedData.split(':');

    const routeMap = {
      grainspawn: '/grain-spawns',
      species: '/species',
      liquidculture: '/liquid-cultures',
      motherculture: '/mother-cultures',
      substrate: '/substrates',
    };

    if (routeMap[type]) {
      navigate(`${routeMap[type]}/${id}`);
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-center">
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <span
              className="font-bold text-2xl bg-linear-to-r from-teal-600 to-cyan-600 
      ...
      <div className={`relative z-50 ${isNavbarOpen ? 'block' : 'hidden'}`}>
        ...
        <nav
          className="fixed top-0 left-0 bottom-0 flex flex-col w-5/6 max-w-sm py-6 px-6 
        ...
        >
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            >
              ...
              <span
                className="font-bold text-xl bg-linear-to-r from-teal-600 to-cyan-600 
              dark:from-teal-400 dark:to-cyan-400 bg-clip-text text-transparent"
              >

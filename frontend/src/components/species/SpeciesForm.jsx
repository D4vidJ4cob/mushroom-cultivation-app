...
export default function SpeciesForm({species, saveSpecies, isEditing}) {
  ...
  const {handleSubmit, touched, errors, getFieldProps, isSubmitting} = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const data = isEditing ? { id: species.id, ...values } : values;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className='flex justify-end gap-3 mt-8'>
        ...
        >
          {isSubmitting ? 'Saving...' : (isEditing ? 'Update Species' : 'Add Species')}
        </button>
      </div>
    </form>
  );

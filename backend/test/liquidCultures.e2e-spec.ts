...
describe('Liquid Cultures', () => {
  let app: INestApplication;
  let drizzle: DatabaseProvider;
  const url = '/api/liquid-cultures';
  ...
  describe('GET /api/liquid-cultures', () => {
    it('should 200 and return all liquid cultures', async () => {
      ...
      expect(names).toEqual(['Lions Mane', 'Chestnut', 'Blue Oyster']);
  ...
  describe('GET /api/liquid-cultures/:id', () => {
    it('should 200 and return liquid culture with related species', async () => {
      const response = await request(app.getHttpServer())
        .get(`${url}/1`)
        .auth(userAuthToken, { type: 'bearer' });
      expect(response.statusCode).toBe(200);
    ...
    it('should 404 when requesting not existing liquid culture', async () => {
      const response = await request(app.getHttpServer())
        .get(`${url}/10`)
        .auth(userAuthToken, { type: 'bearer' });
      expect(response.statusCode).toBe(404);
      expect(response.body.message).toBe(
        'Liquid Culture with ID 10 not found.',
      );
    });
  ...
  describe('POST /api/liquid-cultures', () => {
    it('should 201 and return the created liquid culture', async () => {
      const response = await request(app.getHttpServer())
        .post(url)
        .send({
          name: 'Piopinno',
  ...
  describe('PUT /api/liquid-cultures/:id', () => {
    it('should 200 and return the updated liquid culture', async () => {
      const response = await request(app.getHttpServer())
        .put(`${url}/1`)
        .send({
          name: 'shitake',
    ...
    it('should 404 when updating not existing transaction', async () => {
      ...
      expect(response.statusCode).toBe(404);
      expect(response.body.message).toBe(
        'Liquid Culture with ID 200 not found.',
      );
    });
  ...
  describe('DELETE /api/liquid-cultures/:id', () => {
    ...
    it('should 404 with not existing transaction', async () => {
      ...
      expect(response.statusCode).toBe(404);
      expect(response.body.message).toBe(
        'Liquid Culture with ID 999 not found.',
      );
    });

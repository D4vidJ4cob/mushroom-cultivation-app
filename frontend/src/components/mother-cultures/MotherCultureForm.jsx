...
export default function MotherCultureForm({motherCulture, species, saveMotherCulture, isEditing}) {
  ...
  const {handleSubmit, touched, errors, getFieldProps, isSubmitting } = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const data = isEditing ? { id: motherCulture.id, ...values} : values;

        await saveMotherCulture(data);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className='flex justify-end gap-3 mt-8'>
        ...
        >
          {isSubmitting ? 'Saving...' : (isEditing ? 'Update Mother Culture' : 'Add Mother Culture')}
        </button>
      </div>
    </form>
  );

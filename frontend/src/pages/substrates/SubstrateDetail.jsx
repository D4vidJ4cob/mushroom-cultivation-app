...
import { FaBoxes, FaCalendar, FaSeedling, FaExclamationTriangle, FaUser, FaUsers } from 'react-icons/fa';
import { Link } from 'react-router';

const SubstrateDetail = () => {
  const { id } = useParams();
  
  const { data: substrate, error, isLoading } = useSWR(
    `substrates/${id}`,
    getById,
  );
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  ...
                  dark:from-purple-900/20 dark:to-pink-900/20 border border-purple-200/50 dark:border-purple-700/50">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">Incubation Date</p>
                    </div>
                    <p className="text-gray-800 dark:text-gray-200 font-semibold text-xl">
                ...
                border-amber-200/50 dark:border-amber-700/50 mb-6">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">Source Grain Spawn</p>
                  </div>
                  {substrate.grainSpawn ? (
                    ...
                    >
                      Grain Spawn #{substrate.grainSpawn.id} ({substrate.grainSpawn.species?.name})
                    </Link>
                  ) : (
                    <p className="text-gray-800 dark:text-gray-200 font-semibold text-lg">Unknown</p>
                  )}
                </div>

                {/* Created By */}
                {substrate.userId && (
                  <div className="p-6 rounded-xl bg-teal-50 dark:bg-teal-900/20 border 
                  border-teal-200/50 dark:border-teal-700/50 mb-6">
                    <div className="flex items-center gap-3 mb-2">
                      <FaUser className="w-5 h-5 text-teal-600 dark:text-teal-400" />
                      <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">Created By</p>
                    </div>
                ...
                {substrate.assignments && substrate.assignments.length > 0 && (
                  <div className="p-6 rounded-xl bg-cyan-50 dark:bg-cyan-900/20 border 
                  border-cyan-200/50 dark:border-cyan-700/50">
                    ...
                    <div className="space-y-2">
                      {substrate.assignments.map((assignment) => (
                        <div key={assignment.id} className="p-3 bg-white/50 dark:bg-gray-800/50 rounded-lg 

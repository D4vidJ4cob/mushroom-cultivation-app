...
export default function LiquidCultureForm({
  liquidCulture,
  species,
  saveLiquidCulture,
  isEditing,
      ...
      onSubmit: async (values) => {
        try {
          const payload = isEditing
            ? { id: liquidCulture.id, ...values }
            : values;

          // Save and get the result (which includes the ID for new records)
          const savedLiquidCulture = await saveLiquidCulture(payload);

          // Navigate to detail page
          if (!isEditing && savedLiquidCulture?.id) {
            // For new records: navigate with print prompt
            navigate(`/liquid-cultures/${savedLiquidCulture.id}`, {
              state: { showPrintPrompt: true },
            });
          } else if (isEditing) {
            // For edits: navigate without print prompt
            navigate(`/liquid-cultures/${liquidCulture.id}`);
          } else {

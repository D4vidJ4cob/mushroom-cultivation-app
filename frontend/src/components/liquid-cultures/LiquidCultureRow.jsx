...
export default function LiquidCultureRow({ id, culture, onEdit, onDelete = () => {} } ) {

  const handleDelete = () => {
    onDelete(id);
  };
  
  const formattedDate = new Date(culture.inoculationDate).toLocaleDateString('nl-BE');
  ...
  return (
    ...
    data-cy={`liquid_cultures_row${id}`}>
      ...
      <td className="p-5">
        <span className="text-gray-600 dark:text-gray-400">
          {culture.species.name ? culture.species.name : 'N/A'}
        </span>
      </td>
      ...
      <td className="p-5">
        <div className="flex items-center justify-end gap-2">
          ...
          <button 
            ...
            relative overflow-hidden"
            aria-label="Delete"
            data-cy="delete_liquidCulture_btn" 
          >
            <span className="absolute inset-0 bg-white/20 scale-0 group-hover/btn:scale-100 
            transition-transform duration-300 rounded-xl"></span>

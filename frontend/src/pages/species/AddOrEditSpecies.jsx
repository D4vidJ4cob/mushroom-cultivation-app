...
export default function AddOrEditSpecies() {
  ...
  const {data: species, error: speciesError, isLoading: speciesLoading} = useSWR(
    isEditing ? `species/${id}` : null,
    () => getById(`species/${id}`),
  );

  const {trigger: saveSpecies, error: saveError} = useSWRMutation('species', save);
  ...
  return(
    <div className='w-full min-w-sm bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6'>
      ...
      <AsyncData error={speciesError || saveError} loading={speciesLoading}>
        <SpeciesForm species={species} saveSpecies={saveSpecies} isEditing={isEditing}/>
      </AsyncData>
    </div>
  );
}